  ...
  class FavoritesAPIV2 < Grape::API
    ...
    resource :user do 
      ...
      get '/all_favorites' do
        ...
        favorites = user.favorites.includes(:law_type).where("version > ? and folder_id = ? and visible = ?", params[:version].to_i,params[:folder_id].to_i, true).order('created_at DESC')        
      ...
      post '/favorites' do
        ...
        if count > 0
          { code: 20001, message: '同步失败, error:' + errors.join(', ') }
        else
          { code: 0, message: 'ok' }
        end
